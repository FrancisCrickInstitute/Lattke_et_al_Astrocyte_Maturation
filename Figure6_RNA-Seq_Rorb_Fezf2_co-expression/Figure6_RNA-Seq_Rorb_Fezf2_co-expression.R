...
t1 = read_csv(paste0(in_dir,"/matrix_in_vitro_in_vivo.csv"))
t2 = t1
t2[,-1] = floor(t1[,-1])
mat1 = t2

t1 = read_csv(paste0(in_dir,"/matrix_TF_OE_indiv.csv"))
t2 = t1
t2[,-1] = floor(t1[,-1])
mat2 = t2

t1 = read_csv(paste0(in_dir,"/matrix_TF_OE_comb.csv"))
t2 = t1
t2[,-1] = floor(t1[,-1])
mat3 = t2

count_tab_merged = cbind(mat1[,-1], mat2[,-1],  mat3[,-1])
...
l1[["RF_up"]] = cb$gene[cb$reg_EGFP.vs.Rorb_Fezf2 == -1]
l1[["RF_down"]] = cb$gene[cb$reg_EGFP.vs.Rorb_Fezf2 == 1]

l1[["indiv_R_F_up"]] = intersect(l1$Rorb_up, l1$Fezf2_up)

l1[["comb_RF_R_up"]] = intersect(l1$Rorb_up, l1$RF_up)
l1[["comb_RF_F_up"]] = intersect(l1$Fezf2_up, l1$RF_up)
l1[["comb_RF_R_F_up"]] = intersect(l1$comb_RF_R_up, l1$comb_RF_F_up)
l1[["comb_RF_not_R_F_up"]] = l1$RF_up[!(l1$RF_up %in% c(l1$Rorb_up, l1$Fezf2_up))]

l1[["indiv_R_F_down"]] = intersect(l1$Rorb_down, l1$Fezf2_down)

l1[["comb_RF_R_down"]] = intersect(l1$Rorb_down, l1$RF_down)
l1[["comb_RF_F_down"]] = intersect(l1$Fezf2_down, l1$RF_down)
l1[["comb_RF_R_F_down"]] = intersect(l1$comb_RF_R_down, l1$comb_RF_F_down)
l1[["comb_RF_not_R_F_down"]] = l1$RF_down[!(l1$RF_down %in% c(l1$Rorb_down, l1$Fezf2_down))]
...
#plot selected genes
expr_heatmap(expr_matr_centr, pl_genes = gene_list$mature_low_in_EGFP_comb_RF_not_R_F_up, 
             scale_range = NULL, cellwidth = 14, cellheight = 14, 
             pl_file = paste0(out_dir,"/02_heatmap_mature_low_in_EGFP_comb_RF_not_R_F_up.pdf"))

expr_heatmap(expr_matr_centr, pl_genes = gene_list$immature_high_in_EGFP_comb_RF_not_R_F_down, 
             scale_range = NULL, cellwidth = 14, cellheight = 14, 
             pl_file = paste0(out_dir,"/02_heatmap_immature_high_in_EGFP_comb_RF_not_R_F_down.pdf"))
