...
l1[["FGF_2D_opening"]] = cb$Peak_id[cb$bas_2D.vs.FGF_2D == -1]
l1[["FGF_2D_closing"]] = cb$Peak_id[cb$bas_2D.vs.FGF_2D == 1]
l1[["bas_3D_opening"]] = cb$Peak_id[cb$bas_2D.vs.bas_3D == -1]
l1[["FGF_2D_closing"]] = cb$Peak_id[cb$bas_2D.vs.bas_3D == 1]
l1[["FGF_3D_opening"]] = cb$Peak_id[cb$bas_2D.vs.FGF_3D == -1]
l1[["FGF_3D_closing"]] = cb$Peak_id[cb$bas_2D.vs.FGF_3D == 1]

#only mature opening peaks low in bas_2D
v1 = cb$Peak_id[cb$Astr_P4.vs.Astr_2m == -1 & cb$bas_2D.vs.Astr_2m == -1]
l2 = lapply(l1, intersect, v1)
names(l2) = paste0("mature_opening_low_in_bas_2D_", names(l1))
l2[["mature_opening_low_in_bas_2D"]] = v1
...
peak_label_list_plot = peak_label_list[c("mature_opening_low_in_bas_2D")]
...
for (i in names(peak_label_list_plot)){
  ...
  pdf(file = paste0(out_dir,"/03_coverage_heatmaps_",i,".pdf"), 
      width = length(pl_matrix_list[[i]])*5, height = length(ref_sum_int)/400 +0.5)
  {
