...
t1 = read_csv(paste0(in_dir,"/matrix_TF_OE.csv"))
t2 = t1
t2[,-1] = floor(t1[,-1])
mat2 = t2
...
l1[["Rorb_up"]] = cb$gene[cb$reg_EGFP.vs.Rorb == -1]
l1[["Rorb_down"]] = cb$gene[cb$reg_EGFP.vs.Rorb == 1]
l1[["Dbx2_up"]] = cb$gene[cb$reg_EGFP.vs.Dbx2 == -1]
l1[["Dbx2_down"]] = cb$gene[cb$reg_EGFP.vs.Dbx2 == 1]
l1[["Lhx2_up"]] = cb$gene[cb$reg_EGFP.vs.Lhx2 == -1]
l1[["Lhx2_down"]] = cb$gene[cb$reg_EGFP.vs.Lhx2 == 1]
l1[["Fezf2_up"]] = cb$gene[cb$reg_EGFP.vs.Fezf2 == -1]
l1[["Fezf2_down"]] = cb$gene[cb$reg_EGFP.vs.Fezf2 == 1]
reg_sets = names(l1) #required to extract gene numbers for pairwise proportion test

# mature genes low in EGFP controls regulated by TFs

t1 = read_csv(paste0(in_dir,"/07_Cor vs Str immat mat genes_genes.csv"))

v1 = intersect(na.omit(t1$mat_common), cb$gene[cb$reg_EGFP.vs.Astr_2m == -1])
l2 = lapply(l1, intersect, v1)
names(l2) = paste0("mature_low_in_EGFP_",names(l1))
l2[["mature_low_in_EGFP_reg_by_any_TF"]] = unique(unlist(l2))
l2[["mature_low_in_EGFP"]] = v1

# immature genes high in EGFP controls regulated by TFs

v1 = intersect(na.omit(t1$immat_common), cb$gene[cb$reg_EGFP.vs.Astr_2m == 1])
l3 = lapply(l1, intersect, v1)
names(l3) = paste0("immature_high_in_EGFP_",names(l1))
l3[["immature_high_in_EGFP_reg_by_any_TF"]] = unique(unlist(l3))
l3[["immature_high_in_EGFP"]] = v1

gene_list = c(l1,l2,l3)
...
N_mat_low = c(lengths(l1[c("mature_low_in_EGFP" ,paste0("mature_low_in_EGFP_",reg_sets))]) )
N_immat_high = c(lengths(l1[c("immature_high_in_EGFP" ,paste0("immature_high_in_EGFP_",reg_sets))]) )
...
sink(paste0(out_dir,"/02_genes reg in_immat_high vs all expressed_pairwise comp stat.txt"))
{pairwise.prop.test(N_immat_high, N_expr, p.adjust.method = "BH")}
sink()
...
GOI_tab = read_csv(paste0(in_dir,"/Gene of interest table 21-02-10.csv"))
...
expr_heatmap(expr_matr_centr, pl_genes = gene_list$mature_low_in_EGFP_reg_by_any_TF, 
             scale_range = NULL, cellwidth = 14, cellheight = 14, 
             pl_file = paste0(out_dir,"/02_heatmap_mature_low_in_EGFP_reg_by_any_TF.pdf"))

expr_heatmap(expr_matr_centr, pl_genes = na.omit(GOI_tab$mature_TF_targets_selected), 
             scale_range = NULL, cellwidth = 14, cellheight = 14, 
             pl_file = paste0(out_dir,"/02_heatmap_mature_low_in_EGFP_reg_by_any_TF_selected.pdf"))

expr_heatmap(expr_matr_centr, pl_genes = gene_list$immature_high_in_EGFP_reg_by_any_TF, 
             scale_range = NULL, cellwidth = 14, cellheight = 14, 
             pl_file = paste0(out_dir,"/02_heatmap_immature_high_in_EGFP_reg_by_any_TF.pdf"))

########

############################################################################
# GO analysis of selected modules
############################################################################
...
GO_sets = c("Rorb_up" ,"Rorb_down" , "Dbx2_up" ,  "Dbx2_down" , "Lhx2_up" ,"Lhx2_down",                          
            "Fezf2_up" , "Fezf2_down", "mature_low_in_EGFP_Rorb_up", "mature_low_in_EGFP_Dbx2_up", 
            "mature_low_in_EGFP_Lhx2_up", "mature_low_in_EGFP_Fezf2_up", "mature_low_in_EGFP_reg_by_any_TF"  )

GO_genes_list = l1[GO_sets]

GO_dir = paste0(out_dir,"/02_GO_analysis/") 

if (!dir.exists(GO_dir)) {dir.create(GO_dir)}

#perform GO analysis

GO_list = lapply(GO_genes_list, enrichGO, 
                 OrgDb = org.Mm.eg.db,
                 keyType       = 'SYMBOL',
                 ont           = "BP",
                 pAdjustMethod = "BH",
...
for (i in names(GO_list)){
  write_csv(GO_list[[i]]@result, file = paste0(GO_dir,"/02_GO_",i,".csv"))
}
