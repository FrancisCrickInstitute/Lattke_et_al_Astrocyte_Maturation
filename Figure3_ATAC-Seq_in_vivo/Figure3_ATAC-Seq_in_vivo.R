...
##############################################################################
# basic peak characterisation (overlap with DHS, H3K4me1, differential peaks by peak class)
##############################################################################
...
t1 = deseq_cleaned
t2 = atac_annot[atac_annot$Peak_id %in% t1$Peak_id,]

# overlap of peaks with DHS
t3 = t2 %>% group_by(DHS_pos = (!is.na(DHS_brain_E18)|!is.na(DHS_brain_ad))) %>% 
  summarise(n())
write_csv(t3, file = paste0(out_dir,"/02_DHS_overlap.csv"))

# overlap of distal peaks with H3K4me1
t3 = t2 %>% group_by(peak_type, H3K4me1_pos = (!is.na(H3K4m1_Fo_P0)|!is.na(H3K4m1_Cor_8w))) %>% 
  summarise(n())
write_csv(t3, file = paste0(out_dir,"/02_H3K4me1_overlap.csv"))

### identify opening vs closing peaks
t1 = deseq_cleaned
t1$reg = "not_reg"
t1$reg[(t1$Astr_P4.vs.Astr_2m.log2FoldChange <= -1) & 
         (t1$Astr_P4.vs.Astr_2m.padj <= 0.05)] = "opening"
t1$reg[(t1$Astr_P4.vs.Astr_2m.log2FoldChange >= 1) & 
         (t1$Astr_P4.vs.Astr_2m.padj <= 0.05)] = "closing"
t2 = t1 %>% group_by(peak_type, reg) %>% 
  summarise(n())
write_csv(t2, file = paste0(out_dir,"/02_stats_peaks opening closing by peak type.csv"))
#save bed files for opening vs closing peaks
t2 = bed_cleaned
t3 = t2[t2$X4 %in% t1$peak_label[t1$reg == "opening"],]
write_delim(t3, file = paste0(out_dir,"/02_peaks_opening.bed"), delim = "\t", col_names = FALSE)
t3 = t2[t2$X4 %in% t1$peak_label[t1$reg == "closing"],]
write_delim(t3, file = paste0(out_dir,"/02_peaks_closing.bed"), delim = "\t", col_names = FALSE)
...
t1$atac_reg = 0
t1$atac_reg[(t1$Astr_P4.vs.Astr_2m.atac_padj<= 0.05) & 
                      (t1$Astr_P4.vs.Astr_2m.atac_log2FC <= -1) &
              !is.na(t1$Astr_P4.vs.Astr_2m.atac_padj)] = -1
t1$atac_reg[(t1$Astr_P4.vs.Astr_2m.atac_padj<= 0.05) & 
              (t1$Astr_P4.vs.Astr_2m.atac_log2FC >= 1) &
              !is.na(t1$Astr_P4.vs.Astr_2m.atac_padj)] = 1

t1$rna_reg = 0
t1$rna_reg[(t1$Astr_P4.vs.Astr_2m.rna_padj<= 0.05) & 
              (t1$Astr_P4.vs.Astr_2m.rna_log2FC <= -1) &
              !is.na(t1$Astr_P4.vs.Astr_2m.rna_padj)] = -1
t1$rna_reg[(t1$Astr_P4.vs.Astr_2m.rna_padj<= 0.05) & 
              (t1$Astr_P4.vs.Astr_2m.rna_log2FC >= 1) &
             !is.na(t1$Astr_P4.vs.Astr_2m.rna_padj)] = 1

atac_rna_results_detected_with_bin_comp = t1
write_csv(atac_rna_results_detected_with_bin_comp, path = paste0(out_dir,"/03_ATAC_RNA_merged_detected_with_binary_comparisons.csv"))

### identify immature/mature genes with/without differential peaks 

res_list = NULL

t1 = atac_rna_results_detected_with_bin_comp

t2 = t1[t1$gene %in% na.omit(GOI_tab$immat_common),]
res_list[["immat"]] = t2
res_list[["immat_with_diff_peaks"]] = t2[abs(t2$atac_reg) == 1,]
res_list[["immat_no_diff_peaks"]] = t2[!(t2$gene %in% res_list[["immat_with_diff_peaks"]]$gene),]

t2 = t1[t1$gene %in% na.omit(GOI_tab$mat_common),]
res_list[["mat"]] = t2
res_list[["mat_with_diff_peaks"]] = t2[abs(t2$atac_reg) == 1,]
res_list[["mat_no_diff_peaks"]] = t2[!(t2$gene %in% res_list[["mat_with_diff_peaks"]]$gene),]

write_csv(res_list[["immat_with_diff_peaks"]], file = paste0(out_dir,"/04_reg peaks linked to immat genes.csv"))
write_csv(res_list[["mat_with_diff_peaks"]], file = paste0(out_dir,"/04_reg peaks linked to mat genes.csv"))
...
##############################################################################
# count total number of peaks opening vs closing per gene
##############################################################################
...
t2 = t1 %>% group_by(gene) %>% summarise(N_peaks=n(), 
                                         N_opening= length(atac_reg[atac_reg == -1]),
                                         N_closing= length(atac_reg[atac_reg == 1]))

#calculate net open/closing of peaks
t2$'N_opening-closing' = t2$N_opening - t2$N_closing

peaks_reg_per_gene = t2

write_csv(peaks_reg_per_gene, file = paste0(out_dir,"/04_peaks_reg_per_gene.csv"))

##############################################################################
# analyse and plot total number of peaks opening/closing per gene in immature/mature genes with diff peaks
##############################################################################

# frequency distribution plot table for opening/closing peaks

t1 = NULL

gr = c("immat_with_diff_peaks","mat_with_diff_peaks")

for (i in gr){
  
  t2 = peaks_reg_per_gene[peaks_reg_per_gene$gene %in% res_list[[i]]$gene,]
  t3 = tibble(comparison = i, gene = t2$gene, peak_reg = "opening", 
              N_peaks = t2$N_opening)
  t4 = tibble(comparison = i, gene = t2$gene, peak_reg = "closing", 
              N_peaks = t2$N_closing)
  t1 = rbind(t1, t3, t4 )
  
}

peaks_reg_per_gene_plot_tab = t1


# plot peaks opening/closing per gene by group

t1 = peaks_reg_per_gene_plot_tab

p1 = ggplot(data = t1) + geom_boxplot(aes(x=peak_reg, y = N_peaks, fill = peak_reg),
                                      outlier.size = 0.5) +
  facet_wrap(~comparison)+
  scale_fill_manual(limits = unique(t1$peak_reg), values = c("red", "blue"))+
  labs(x = "", y = "peaks_per_gene", fill = "")+
  theme(text =  element_text(size = 12, face = 2),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))

pdf(file = paste0(out_dir,"/04_peaks_per_gene_boxplot.pdf"), width = 5, height = 3)
{
  p1
}    
dev.off()
...
# summary numbers of peak changes per gene by comparison; stat peaks opening vs closing

t1 = peaks_reg_per_gene_plot_tab

t2 = t1 %>% group_by(comparison, peak_reg) %>% 
  summarise(min = quantile(N_peaks)[1],
            q1 = quantile(N_peaks)[2],
            median = quantile(N_peaks)[3],
            q3 = quantile(N_peaks)[4],
            max = quantile(N_peaks)[5])

write_csv(t2, path = paste0(out_dir,"/04_peaks_reg_per_gene_quantile_summary.csv"))

t1 = peaks_reg_per_gene_plot_tab
t2 = t1 %>% group_by(comparison) %>% summarise(wilcox.test(N_peaks ~ peak_reg)$p.value)
write_csv(t2, path = paste0(out_dir,"/04_peaks_reg_opening_vs_closing_by_group_wilcoxon_p-val.csv"))


### Boxplot and stats of net peak opening (peaks opening - peaks closing by gene)
...
for (i in gr){
  ...
  t2 = peaks_reg_per_gene[peaks_reg_per_gene$gene %in% res_list[[i]]$gene,]
  t3 = tibble(comparison = i, gene = t2$gene, peak_reg = "opening-closing", 
              N_peaks = t2$`N_opening-closing`)
  t1 = rbind(t1, t3)
...
p1 = ggplot(data = t1) + geom_boxplot(aes(x=comparison, y = N_peaks, fill = comparison),
                                      outlier.size = 0.5) +
  scale_fill_manual(limits = gr, values = c("green", "darkgreen"))+
  scale_x_discrete(limits = gr, labels = str_remove(gr,"_with_diff_peaks"))+
  labs(x = "", y = "opening - closing peaks", fill = "")+
  theme(text =  element_text(size = 12, face = 2),
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))

pdf(file = paste0(out_dir,"/04_peaks_reg_net_opening_per_gene_boxplot.pdf"), width = 3.5, height = 3)
{
  p1
}    
dev.off()
...
write_csv(t2, path = paste0(out_dir,"/04_peaks_reg_net_opening_per_gene_quantile_summary.csv"))


t1 = peaks_reg_per_gene_plot_tab
t2 = t1 %>% summarise(wilcox.test(N_peaks ~ comparison)$p.value)
write_csv(t2, path = paste0(out_dir,"/04_peaks_reg_net_opening-closing_by_group_wilcoxon_p-val.csv"))
...
##############################################################################
##############################################################################
# [5] analyse chromatin accessibility changes at promoters vs enhancers linked to immature/mature genes
##############################################################################
##############################################################################
...
### identify immature/mature genes with/without differential promoters/enhancers
...
t2 = t1[t1$gene %in% na.omit(GOI_tab$immat_common),]
res_list[["immat"]] = t2
res_list[["immat_with_diff_promoters"]] = t2[abs(t2$atac_reg) == 1 & 
                                               t2$peak_type == "Promoter",]
res_list[["immat_with_opening_promoters"]] = t2[t2$atac_reg == -1 & 
                                               t2$peak_type == "Promoter",]
res_list[["immat_with_closing_promoters"]] = t2[t2$atac_reg == 1 & 
                                                  t2$peak_type == "Promoter",]
res_list[["immat_with_diff_enhancers"]] = t2[abs(t2$atac_reg) == 1 & 
                                               t2$peak_type == "Enhancer",]
res_list[["immat_with_opening_enhancers"]] = t2[t2$atac_reg == -1 & 
                                               t2$peak_type == "Enhancer",]
res_list[["immat_with_closing_enhancers"]] = t2[t2$atac_reg == 1 & 
                                                  t2$peak_type == "Enhancer",]
res_list[["immat_with_opening_and_closing_enhancers"]] = t2[(t2$gene %in% res_list[["immat_with_opening_enhancers"]]$gene) &
                                                              ((t2$gene %in% res_list[["immat_with_closing_enhancers"]]$gene)),]

t2 = t1[t1$gene %in% na.omit(GOI_tab$mat_common),]
res_list[["mat"]] = t2
res_list[["mat_with_diff_promoters"]] = t2[abs(t2$atac_reg) == 1 &
                                          t2$peak_type == "Promoter",]
res_list[["mat_with_opening_promoters"]] = t2[t2$atac_reg == -1 & 
                                                  t2$peak_type == "Promoter",]
res_list[["mat_with_closing_promoters"]] = t2[t2$atac_reg == 1 & 
                                                  t2$peak_type == "Promoter",]
res_list[["mat_with_diff_enhancers"]] = t2[abs(t2$atac_reg) == 1 & 
                                               t2$peak_type == "Enhancer",]
res_list[["mat_with_opening_enhancers"]] = t2[t2$atac_reg == -1 & 
                                                  t2$peak_type == "Enhancer",]
res_list[["mat_with_closing_enhancers"]] = t2[t2$atac_reg == 1 & 
                                                  t2$peak_type == "Enhancer",]
res_list[["mat_with_opening_and_closing_enhancers"]] = t2[(t2$gene %in% res_list[["mat_with_opening_enhancers"]]$gene) &
                                                              ((t2$gene %in% res_list[["mat_with_closing_enhancers"]]$gene)),]
...
write_csv(t1, file = paste0(out_dir,"/05_stats_immat mat genes with vs without diff promoters vs enhancers.csv"))
...
##############################################################################
##############################################################################
# [6] analyse chromatin accessibility changes at all peaks linked to immature/mature genes 
#       (with 100kb cutoff)
##############################################################################
##############################################################################
...
#keep only peak-gene-connections from 100kb cutoff linked genes
t0 = atac_rna_results_detected_with_bin_comp
t1 = t0[t0$linked_gene_100kb == t0$gene,]
...
write_csv(t1, file = paste0(out_dir,"/06_stats_immat mat genes with vs without diff peaks_100kb_cutoff.csv"))
...
write_csv(peaks_reg_per_gene, file = paste0(out_dir,"/06_peaks_reg_per_gene_100kb cutoff.csv"))
...
pdf(file = paste0(out_dir,"/06_peaks_per_gene_boxplot_100kb cutoff.pdf"), width = 5, height = 3)
{
  p1
}    
dev.off()
...
write_csv(t2, path = paste0(out_dir,"/06_peaks_reg_per_gene_quantile_summary_100kb cutoff.csv"))

t1 = peaks_reg_per_gene_plot_tab
t2 = t1 %>% group_by(comparison) %>% summarise(wilcox.test(N_peaks ~ peak_reg)$p.value)
write_csv(t2, path = paste0(out_dir,"/06_peaks_reg_opening_vs_closing_by_group_wilcoxon_p-val_100kb cutoff.csv"))
...
pdf(file = paste0(out_dir,"/06_peaks_reg_net_opening_per_gene_boxplot_100kb cutoff.pdf"), width = 3.5, height = 3)
{
  p1
}    
dev.off()
...
write_csv(t2, path = paste0(out_dir,"/06_peaks_reg_net_opening_per_gene_quantile_summary_100kb cutoff.csv"))


t1 = peaks_reg_per_gene_plot_tab
t2 = t1 %>% summarise(wilcox.test(N_peaks ~ comparison)$p.value)
write_csv(t2, path = paste0(out_dir,"/06_peaks_reg_net_opening-closing_by_group_wilcoxon_p-val_100kb cutoff.csv"))
